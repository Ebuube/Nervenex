...
$(function () {
	...
	function viewHistory() {
		clickedAttempt = true;
		window.location.href = `http://web-01.brainspark.tech.tech/nervenex/correction/${this.id}`;
	}
	...
	$("li.quiz_item").click(function takeQuiz() {
		...
		$.ajax({
			url: `http://web-01.brainspark.tech.tech/nervenex/api/v1/quizzes/${quiz_id}`,
			type: 'GET',
			dataType: 'json',
		})
		.done(function (Quiz) {
			...
			setTimeout(() => {
				window.location.href = `http://web-01.brainspark.tech.tech/nervenex/quiz/${quiz_id}`;
			}, 2000);
		})
